...
export function PeriodicTableTab() {
  ...
  return (
    <div className="space-y-6">
      ...
      {elementInfo && (
        <div className="bg-[#F0EDEB] border border-[#DFC1BF] rounded-lg p-6 max-w-md">
          <div className="flex items-baseline gap-3 mb-4">
            <span className="text-4xl font-bold text-gray-900">{elementInfo.symbol}</span>
            <span className="text-[#5A6B63]">{elementInfo.name}</span>
          </div>
          <div className="grid grid-cols-2 gap-3 text-sm">
            <div>
              <span className="text-[#5A6B63]">Atomic Number</span>
              <p className="font-mono text-gray-900">{elementInfo.Z}</p>
            </div>
            <div>
              <span className="text-[#5A6B63]">Atomic Mass</span>
              <p className="font-mono text-gray-900">{elementInfo.atomic_mass.toFixed(4)} u</p>
            </div>
          </div>
          {elementEdges.length > 0 && (
            <div className="mt-4">
              <span className="text-[#5A6B63] text-sm">Key Edges</span>
              <div className="flex flex-wrap gap-2 mt-1">
                {elementEdges.slice(0, 5).map((edge) => (
                  <span key={edge.edge} className="px-2 py-1 bg-[#DFC1BF] rounded text-xs font-mono text-gray-800">
